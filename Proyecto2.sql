...
(id_matricula number generated by default on null as identity,
...
create or replace function obtener_requisito(n_id_materia number)
return number is
id_requisito number := 0;
begin

select id_materia_requisito into id_requisito from requisito_materia where id_materia = n_id_materia;
...
--select obtener_requisito(1) from dual;

--funcion para retornar id_materia de un grupo especifico

create or replace function obtener_materia( n_grupo number)
return number is
id_ma number;
begin
...
create or replace function verificar_aprobacion(n_carnet number, n_id_materia number)
return number is
resultado number;
nota number;
prueba number;
...
select id_grupo_matriculado, nota_final from periodo where carnet_estudiante = n_carnet;

begin
prueba := 0;
id_grupo := 0;
for reg in cur_historial
loop
...
select max(nota_final) into nota from periodo where id_grupo_matriculado = id_grupo and carnet_estudiante = n_carnet;
...
--select verificar_aprobacion(20170001, 2) from dual; 


--procedimiento para matricular en un grupo

create or replace procedure matricular_estudiante(n_carnet number, n_grupo number)
is
id_materia number;
requisito number;
aprobacion number;
...
insert into matricula values (null, n_carnet, n_grupo);

else
  dbms_output.put_line('No puede matricular el grupo ' || n_grupo || ' porque no cumple con el requisito');
